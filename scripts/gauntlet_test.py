...
GAUNTLET: list[tuple[str, str, dict, int | None]] = [
    ...
    ("17 original_zip_pw",      "0501961423",    {"dates": ((1, 5, 1996),), "stems": ("isinho", "satabola")}, 3),
...
def build_hints(hints_dict: dict) -> "Hints":
...
async def main(stop_on_fail: bool, from_index: int, stages: list[int] | None, budget: float) -> None:
    print("\n" + "=" * 70)
    ...
    for i, (label, password, hints_dict, expected_stage) in enumerate(GAUNTLET):
        ...
        print(f"[{label}]  pw={password!r:<22}  expected=stage{expected_stage or '??'}", end=" ", flush=True)

        t0 = time.perf_counter()
        try:
            res = await asyncio.wait_for(
                run_one(label, archive, hints_dict, budget_s=budget, allowed_stages=stages),
                timeout=budget + 10,
            )
        except asyncio.TimeoutError:
            res = Result(label=label, password=password, error="TIMEOUT", elapsed_s=budget + 10)
        ...
        print(f"  {icon}  {elapsed:.1f}s  {'FOUND: ' + res.output if res.found else res.error or 'NOT FOUND (' + res.output + ')'}")
    ...
    if failed:
        ...
        for r in results:
            if not r.found:
                print(f"  {r.label:35s}  pw={r.password!r}  {'ERROR: '+r.error if r.error else 'not found'}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stop-on-fail", action="store_true", help="Stop on first unexpected failure")
    parser.add_argument("--from", dest="from_index", type=int, default=1, help="Start from test N")
    parser.add_argument("--stages", default=None, help="Comma-separated stage numbers to run (e.g. 1,3,4)")
    parser.add_argument("--budget", type=float, default=60.0, help="Per-test time budget in seconds (default 60)")
